...
OUTPUT_DIR="${TEST_OUTPUT_DIR:-$HOME/Downloads/FanControl-Automatic-Install-Test-$(date +%Y%m%d-%H%M%S)}"
...
xcodebuild -project "$PROJECT_DIR/FanControl.xcodeproj" -scheme FanControl \
    ...
    archive -archivePath "$WORK_DIR/FanControl.xcarchive" -allowProvisioningUpdates
